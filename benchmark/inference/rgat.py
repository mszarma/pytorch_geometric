...
class GAT_HETERO:
    def __init__(self, hidden_channels, output_channels, num_layers, num_heads) -> None:
        self.model = None
    ...
    def create_hetero(self, device, metadata):
        model = GAT_FOR_HETERO(
            self.hidden_channels, self.output_channels, self.num_layers, self.num_heads)
...
class GAT_FOR_HETERO(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, num_layers, heads):
        ...
        for i in range(num_layers - 2):
            self.convs.append(GATConv((-1, -1), hidden_channels,
                              heads=heads, add_self_loops=False))
        self.convs.append(GATConv((-1, -1), out_channels,
                          heads=heads, add_self_loops=False))
